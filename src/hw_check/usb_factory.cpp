 ...
 * @author: LiJun
 * @date: 2026-04-21
 * @fileName: usb_factory.cpp
 * @description: hw_check .so 导出符号实现（usb 版本）
 *
 * 编译到 libhw_check_usb.so 中，仅包含 usb 检测实现。
 */
...
static struct hw_check_ctx *usb_create(enum hw_check_type type,
                                       hw_check_notify_fn notify,
                                       void *user_data,
                                       bool run_thread_check)
{
    ...
    impl->obj = new usb_check(nullptr, run_thread_check);

    return reinterpret_cast<struct hw_check_ctx*>(impl);
}

static void usb_destroy(struct hw_check_ctx *ctx)
{
    if (!ctx) return;
...
static bool usb_get_exist(struct hw_check_ctx *ctx)
{
    if (!ctx) return false;
    auto *impl = reinterpret_cast<hw_check_ctx_impl*>(ctx);
...
static struct hw_check_opt usb_opt = {
    .create    = usb_create,
    .destroy   = usb_destroy,
    .get_exist = usb_get_exist,
};

static struct hw_check_intf usb_intf = {
    .name = "usb",
    .opt  = &usb_opt,
};
